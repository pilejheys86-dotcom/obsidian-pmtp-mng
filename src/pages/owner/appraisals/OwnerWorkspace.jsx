...
import { Pagination, StatsCard, StatusBadge, StepNav, EmptyState, Modal } from '../../../components/ui'
import { getNavigationByRole } from '../../../config'
import { useAuth } from '../../../context'
import { appraisalsApi, customersApi, pricingApi, loanSettingsApi } from '../../../lib/api'
...
const STATUS_MAP = {
  ...
  UNDER_APPRAISAL: { label: 'Under Appraisal', type: 'info' },
...
const STATUS_FILTERS = [
  { value: '', label: 'All' },
  { value: 'PENDING_APPRAISAL', label: 'Pending Appraisal' },
...
const INITIAL_FORM = {
  category: '', metal_type: '', general_desc: '', item_condition: '',
  weight_grams: '', karat: '', brand: '', model: '', serial_number: '',
  gadget_color: '', storage_capacity: '',
  appliance_brand: '', appliance_model: '', appliance_serial: '',
  size_capacity: '', wattage: '', appliance_color: '',
  vehicle_make: '', vehicle_model: '', vehicle_year: '', vehicle_color: '',
  plate_number: '', engine_number: '', chassis_number: '', mileage: '',
  transmission: '', fuel_type: '', accessories: [], appraised_value: '',
}
...
export default function OwnerWorkspace() {
  ...
  const [view, setView] = useState('list') // 'list' | 'submit' | 'print'
  const [statusFilter, setStatusFilter] = useState('')

  // ── List state
  const [stats, setStats] = useState({ pendingApproval: 0, readyForRelease: 0, issuedToday: 0, rejected: 0 })
  ...
  const [loanSettings, setLoanSettings] = useState({ ltv_ratio: 0.70 })

  // Fetch item conditions and loan settings
  useEffect(() => {
    pricingApi.getItemConditions()
      .then(data => setConditions((data || []).filter(c => c.is_active)))
      .catch(() => {})
    loanSettingsApi.get()
      .then(data => { if (data) setLoanSettings(data) })
      .catch(() => {})
  }, [])
  ...
  const completedSteps = useMemo(() => {
    ...
    let itemExtra = true
    if (formData.category === 'JEWELRY') itemExtra = !!(formData.weight_grams && formData.karat)
    else if (formData.category === 'GADGET') itemExtra = !!(formData.brand.trim() && formData.model.trim() && formData.serial_number.trim())
  ...
  const fetchQueue = async () => {
    try {
      ...
      console.error('Owner workspace fetch error:', err)
    } finally {
      setLoading(false)
    }
  }
      ...
      try {
        ...
        results.sort((a, b) => {
          const score = (c) => {
            ...
            let s = 0
            if (full === q) s += 100
            if (full.startsWith(q)) s += 50
            for (const w of words) { if (first.startsWith(w)) s += 20; if (last.startsWith(w)) s += 20; if (first.includes(w)) s += 5; if (last.includes(w)) s += 5 }
            return s
          }
  ...
  useEffect(() => {
    ...
    let rafId
    const sc = document.querySelector('.custom-scrollbar') || window
    const handle = () => {
      cancelAnimationFrame(rafId)
      rafId = requestAnimationFrame(() => {
        let current = FORM_STEPS[0]?.id
        for (const s of FORM_STEPS) { const el = document.getElementById(s.id); if (el && el.getBoundingClientRect().top <= 120) current = s.id }
        setActiveStep((p) => p === current ? p : current)
      })
    }
    const t = setTimeout(handle, 150)
    sc.addEventListener('scroll', handle, { passive: true })
    return () => { clearTimeout(t); cancelAnimationFrame(rafId); sc.removeEventListener('scroll', handle) }
  }, [view])
  ...
  const handleAccessoryToggle = (acc) => {
    setFormData((prev) => ({ ...prev, accessories: prev.accessories.includes(acc) ? prev.accessories.filter((a) => a !== acc) : [...prev.accessories, acc] }))
  }

  const handleCalculate = async () => {
    try {
      setCalcLoading(true)
      const res = await appraisalsApi.calculate({ weight_grams: Number(formData.weight_grams), karat: Number(formData.karat), item_condition: formData.item_condition })
      setCalcResult(res)
      handleFormChange('appraised_value', res.appraised_value || '')
    } catch (err) { console.error('Calculate error:', err) }
  ...
  const handleSubmitAppraisal = async () => {
    try {
      ...
      await appraisalsApi.submit({
        customer_id: selectedCustomer.id, category: formData.category, description: formData.general_desc,
        condition: formData.item_condition, accessories: formData.accessories, appraised_value: Number(formData.appraised_value),
        fair_market_value: Number(formData.appraised_value),
        brand: formData.brand || null, model: formData.model || null, serial_number: formData.serial_number || null,
        weight_grams: formData.weight_grams ? Number(formData.weight_grams) : null, karat: formData.karat ? Number(formData.karat) : null,
  ...
  const handleApprove = async () => {
    try {
      ...
      await appraisalsApi.approve(approveModal.item.id, { principal_loan: Number(principalLoan), offered_amount: offeredAmount ? Number(offeredAmount) : null, storage_location: storageLocation || null })
      setApproveModal({ open: false, item: null }); setPrincipalLoan(''); setOfferedAmount(''); setStorageLocation('')
      fetchQueue()
  ...
  const handleReject = async () => {
    try {
      ...
      setRejectModal({ open: false, item: null }); setRejectReason('')
      fetchQueue()
  ...
  const handleDecline = async () => {
    try {
      setModalLoading(true)
      await appraisalsApi.decline(declineModal.item.id, { reason: declineReason })
      setDeclineModal({ open: false, item: null }); setDeclineReason('')
      fetchQueue()
  ...
  const handleIssueSuccess = (result) => {
    setPrintTicket(result)
    setPrintItem(issueItem)
    setIssueItem(null)
    setView('print')
  }

  const resetForm = () => {
    setActiveStep('customer'); setCustomerSearch(''); setCustomerResults([]); setSelectedCustomer(null)
    setFormData(INITIAL_FORM); setCalcResult(null); setSelectedItem(null)
  }

  // ── KPI
  const statsData = [
    { icon: 'approval', iconBg: 'bg-blue-500', iconColor: 'text-white', label: 'Pending Approval', value: String(stats.pendingApproval || 0) },
    { icon: 'local_shipping', iconBg: 'bg-emerald-500', iconColor: 'text-white', label: 'Ready for Release', value: String(stats.readyForRelease || 0) },
    { icon: 'receipt_long', iconBg: 'bg-primary', iconColor: 'text-white', label: 'Issued Today', value: String(stats.issuedToday || 0) },
      ...
      <div className="admin-layout">
        ...
        <main className="admin-main">
          ...
          <div className="admin-content custom-scrollbar">
            <div className="flex items-center gap-4 mb-6 print-hide">
              <button onClick={() => { setView('list'); setPrintTicket(null); setPrintItem(null) }} className="btn-secondary">
                <span className="material-symbols-rounded text-lg">arrow_back</span>Back to Queue
              </button>
              <button onClick={() => window.print()} className="btn-primary">
                <span className="material-symbols-rounded text-lg">print</span>Print Ticket
              </button>
            </div>
  ...
  return (
    <div className="admin-layout">
        ...
        <div className="admin-content custom-scrollbar">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <h1 className="text-2xl font-display font-bold text-neutral-800 dark:text-neutral-100">
                {view === 'submit' ? 'New Appraisal' : 'Appraisal Management'}
              </h1>
            </div>
            <div className="flex items-center gap-4">
              {view === 'list' ? (
                <button className="btn-primary" onClick={() => { resetForm(); setView('submit') }}>
                  <span className="material-symbols-rounded text-lg">add</span>New Appraisal
                </button>
              ) : (
              ...
              <div className="flex flex-col gap-6">
                ...
                <section id="customer" className="rounded-sm border shadow-sm scroll-mt-24 bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700">
                  <div className="px-6 pt-5 pb-4 border-b border-neutral-100 dark:border-neutral-700/60">
                    <div className="flex items-center gap-2.5">
                      ...
                      <h2 className="text-[15px] font-bold text-neutral-800 dark:text-neutral-100">Select Customer</h2>
                    </div>
                    <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-1 ml-[38px]">Search for an existing customer to associate with this appraisal.</p>
                  </div>
                  <div className="p-6 space-y-5">
                    {selectedCustomer ? (
                      <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg flex items-center gap-3">
                        ...
                        <button type="button" onClick={() => { setSelectedCustomer(null); setCustomerSearch(''); setCustomerResults([]) }} className="text-neutral-400 hover:text-red-500 transition-colors">
                      ...
                      <>
                        ...
                        {customerLoading && (
                          <div className="flex items-center gap-2 py-4 text-sm text-neutral-500 dark:text-neutral-400">
                            <span className="material-symbols-rounded animate-spin text-base">progress_activity</span>Searching customers...
                          </div>
                        )}
                        ...
                        {!customerLoading && customerSearch.trim() && customerResults.length === 0 && (
                          <div className="text-sm text-neutral-400 dark:text-neutral-500 py-4 text-center">No customers found for &ldquo;{customerSearch}&rdquo;</div>
                        )}
                      </>
                    )}
                  </div>
                    ...
                    {formData.category && (<>
                      {formData.category === 'JEWELRY' && (<>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 p-4 bg-neutral-50 dark:bg-neutral-700/30 rounded-lg border border-neutral-200/60 dark:border-neutral-700/50">
                          <div><label className="form-label">Weight (grams)</label><input type="number" value={formData.weight_grams} onChange={(e) => handleFormChange('weight_grams', e.target.value)} className="form-input w-full" placeholder="e.g. 15.5" min="0" step="0.01" /></div>
                          <div><label className="form-label">Karat</label><select value={formData.karat} onChange={(e) => handleFormChange('karat', e.target.value)} className="form-input w-full"><option value="">Select karat...</option>{KARATS.map((k) => <option key={k} value={k}>{k}K</option>)}</select></div>
                        </div>
                      </>)}
                      ...
                      {formData.category === 'VEHICLE' && (
                        <div className="space-y-4 p-4 bg-neutral-50 dark:bg-neutral-700/30 rounded-lg border border-neutral-200/60 dark:border-neutral-700/50">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div><label className="form-label">Transmission</label><select value={formData.transmission} onChange={(e) => handleFormChange('transmission', e.target.value)} className="form-input w-full"><option value="">Select...</option><option value="AUTOMATIC">Automatic</option><option value="MANUAL">Manual</option><option value="CVT">CVT</option></select></div>
                            <div><label className="form-label">Fuel Type</label><select value={formData.fuel_type} onChange={(e) => handleFormChange('fuel_type', e.target.value)} className="form-input w-full"><option value="">Select...</option><option value="GASOLINE">Gasoline</option><option value="DIESEL">Diesel</option><option value="ELECTRIC">Electric</option><option value="HYBRID">Hybrid</option></select></div>
                          </div>
                        </div>
                      )}

                      <div><label className="form-label">Item Condition</label><select value={formData.item_condition} onChange={(e) => handleFormChange('item_condition', e.target.value)} className="form-input w-full"><option value="">Select condition...</option>{conditions.map((c) => <option key={c.condition_name} value={c.condition_name}>{c.condition_name}</option>)}</select></div>
                        ...
                        {formData.category === 'OTHER' ? (
                          <div className="space-y-3 mt-2">
                            <div className="flex gap-2">
                              <input type="text" value={customAccessory} onChange={(e) => setCustomAccessory(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); const val = customAccessory.trim(); if (val && !formData.accessories.includes(val)) setFormData((p) => ({ ...p, accessories: [...p.accessories, val] })); setCustomAccessory('') } }} className="form-input w-full" placeholder="Type an accessory and press Enter..." />
                              <button type="button" onClick={() => { const val = customAccessory.trim(); if (val && !formData.accessories.includes(val)) setFormData((p) => ({ ...p, accessories: [...p.accessories, val] })); setCustomAccessory('') }} className="px-4 rounded-lg text-sm font-medium bg-primary/10 text-primary border border-primary/30 hover:bg-primary/20 transition-colors whitespace-nowrap">Add</button>
                            </div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {formData.accessories.map((acc) => (
                                  <span key={acc} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-primary/10 border border-primary/30 text-primary">
                                    {acc}
                                    <button type="button" onClick={() => setFormData((p) => ({ ...p, accessories: p.accessories.filter((a) => a !== acc) }))} className="text-primary/60 hover:text-red-500 transition-colors"><span className="material-symbols-rounded text-[14px]">close</span></button>
                                  </span>
                                ))}
                              </div>
                            )}
                ...
                <section id="valuation" className="rounded-sm border shadow-sm scroll-mt-24 bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700">
                  <div className="px-6 pt-5 pb-4 border-b border-neutral-100 dark:border-neutral-700/60">
                    ...
                    <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-1 ml-[38px]">{formData.category === 'JEWELRY' ? 'Calculate the appraised value based on weight and karat, or enter manually.' : 'Enter the appraised value for this item.'}</p>
                  </div>
                  <div className="p-6 space-y-5">
                    {formData.category === 'JEWELRY' && (
                  ...
                  <div className="p-6 space-y-6">
                    <div className="rounded-xl border border-neutral-200/80 dark:border-neutral-700/60 overflow-hidden">
                      <div className="px-4 py-2.5 bg-neutral-50 dark:bg-neutral-700/20 border-b border-neutral-200/80 dark:border-neutral-700/60">
                        <h3 className="text-[11px] font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider flex items-center gap-1.5"><span className="material-symbols-rounded text-[13px] text-primary">person</span>Customer</h3>
                      </div>
                      <div className="px-4 py-3.5 flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center shrink-0"><span className="text-primary font-bold text-xs">{(selectedCustomer?.first_name?.[0] || '').toUpperCase()}{(selectedCustomer?.last_name?.[0] || '').toUpperCase()}</span></div>
                        <div className="min-w-0"><p className="text-[13px] font-semibold text-neutral-800 dark:text-neutral-100 truncate">{selectedCustomer?.first_name} {selectedCustomer?.last_name}</p><p className="text-[11px] text-neutral-400 dark:text-neutral-500">ID: {selectedCustomer?.id?.slice(0, 8)}</p></div>
                      </div>
                    </div>
                    <div className="rounded-xl border border-neutral-200/80 dark:border-neutral-700/60 overflow-hidden">
                      <div className="px-4 py-2.5 bg-neutral-50 dark:bg-neutral-700/20 border-b border-neutral-200/80 dark:border-neutral-700/60">
                        <h3 className="text-[11px] font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider flex items-center gap-1.5"><span className="material-symbols-rounded text-[13px] text-primary">inventory_2</span>Item Details</h3>
                      </div>
                      <div className="px-4 py-3.5 space-y-2.5">
                        ...
                        {formData.general_desc && (<div className="pt-1 border-t border-neutral-100 dark:border-neutral-700/40"><p className="text-[11px] text-neutral-400 dark:text-neutral-500 mb-0.5">Description</p><p className="text-[13px] text-neutral-700 dark:text-neutral-200">{formData.general_desc}</p></div>)}
                        {formData.accessories.length > 0 && (
                          <div className="pt-2 border-t border-neutral-100 dark:border-neutral-700/40">
                            ...
                            <div className="flex flex-wrap gap-1.5">{formData.accessories.map((acc) => (<span key={acc} className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-medium bg-neutral-100 dark:bg-neutral-700/40 text-neutral-600 dark:text-neutral-300 border border-neutral-200/60 dark:border-neutral-600/30">{acc}</span>))}</div>
                          </div>
                        )}
                      </div>
                    </div>
                    <div className="rounded-xl border border-primary/15 dark:border-primary/20 bg-gradient-to-r from-primary/5 to-primary/[0.02] dark:from-primary/10 dark:to-primary/[0.03] overflow-hidden">
                      <div className="px-4 py-2.5 border-b border-primary/10 dark:border-primary/15"><h3 className="text-[11px] font-semibold text-primary/70 uppercase tracking-wider flex items-center gap-1.5"><span className="material-symbols-rounded text-[13px] text-primary">payments</span>Appraised Value</h3></div>
                      <div className="px-4 py-4"><p className="text-2xl font-bold text-primary tracking-tight">{formatCurrency(formData.appraised_value)}</p></div>
                    </div>
                  </div>
                </section>
          ...
          {view === 'list' && (
            <>
              ...
              {/* Status filter tabs */}
              <div className="flex items-center gap-1 mb-6 border-b border-neutral-200 dark:border-neutral-700 overflow-x-auto">
                {STATUS_FILTERS.map((sf) => (
                  <button key={sf.value} onClick={() => { setStatusFilter(sf.value); setCurrentPage(1) }}
                    className={`px-4 py-2.5 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${statusFilter === sf.value ? 'border-primary text-primary' : 'border-transparent text-neutral-500 dark:text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200'}`}>
                    {sf.label}
                  </button>
                ))}
              </div>
                ...
                <div className="overflow-x-auto custom-scrollbar flex-1">
                  {loading ? (
                    <div className="flex items-center justify-center py-16 text-neutral-400 dark:text-neutral-500">
                      <span className="material-symbols-rounded animate-spin text-2xl mr-2">progress_activity</span>Loading appraisals...
                    </div>
                  ) : queue.length === 0 ? (
                    <EmptyState icon="assignment" title="No appraisals found" description="No items matching your filter." />
                  ) : (
                    <table className="min-w-full text-center text-sm whitespace-nowrap">
                      ...
                      <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                        {queue.map((item) => {
                          ...
                          const hasLoanTerms = !!item.specific_attrs?.loan_terms
                          const needsAppraisal = item.inventory_status === 'PENDING_APPRAISAL'
                          const isPending = item.inventory_status === 'APPRAISED' && !hasLoanTerms
                          const isReady = item.inventory_status === 'APPRAISED' && hasLoanTerms
                          ...
                          return (
                            <tr key={item.id} className="loan-row">
                              <td className="px-4 py-4 text-center">
                                ...
                                {(item.brand || item.serial_number) && (<p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">{[item.brand, item.model].filter(Boolean).join(' ')}{item.serial_number && ` \u00B7 S/N: ${item.serial_number}`}</p>)}
                              </td>
                                ...
                                <div className="flex items-center justify-center gap-2 flex-wrap">
                                  {needsAppraisal && (
                                    <button onClick={() => {
                                      ...
                                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 hover:bg-amber-500/20 transition-colors">
                                      <span className="material-symbols-rounded text-sm">rate_review</span>Start Appraisal
                                    </button>
                                  )}
                                  {isPending && (
                                    <>
                                      <button onClick={() => { const fmv = Number(item.fair_market_value || item.appraised_value); setPrincipalLoan(Math.round(fmv * (loanSettings.ltv_ratio || 0.70) * 100) / 100); setOfferedAmount(''); setStorageLocation(''); setApproveModal({ open: true, item }) }}
                                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 transition-colors">
                                        <span className="material-symbols-rounded text-sm">check_circle</span>Approve
                                      </button>
                                      <button onClick={() => { setRejectReason(''); setRejectModal({ open: true, item }) }}
                                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20 hover:bg-red-500/20 transition-colors">
                                        <span className="material-symbols-rounded text-sm">cancel</span>Reject
                                      </button>
                                    </>
                                  )}
                                  {isReady && (
                                    <>
                                      <button onClick={() => setIssueItem(item)}
                                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 transition-colors">
                                        <span className="material-symbols-rounded text-sm">receipt_long</span>Issue Ticket
                                      </button>
                                      <button onClick={() => { setDeclineReason(''); setDeclineModal({ open: true, item }) }}
                                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-neutral-500/10 text-neutral-600 dark:text-neutral-400 border border-neutral-500/20 hover:bg-neutral-500/20 transition-colors">
                                        <span className="material-symbols-rounded text-sm">person_off</span>Decline
                                      </button>
                                    </>
                                  )}
                                  {!needsAppraisal && !isPending && !isReady && <span className="text-xs text-neutral-400 dark:text-neutral-500">---</span>}
                                </div>
                              </td>
                            </tr>
                          )
        ...
        {approveModal.item && (
          <div className="space-y-5">
            <div className="p-3 bg-neutral-50 dark:bg-neutral-700/30 rounded-lg space-y-1">
              ...
              <p className="text-xs text-neutral-500 dark:text-neutral-400">Fair Market Value: <span className="font-bold text-primary">{formatCurrency(approveModal.item.fair_market_value || approveModal.item.appraised_value)}</span> | Max Loan (LTV {((loanSettings.ltv_ratio || 0.70) * 100).toFixed(0)}%): <span className="font-bold">{formatCurrency((approveModal.item.fair_market_value || approveModal.item.appraised_value) * (loanSettings.ltv_ratio || 0.70))}</span></p>
            </div>
            <div>
              <label className="form-label">Principal Loan Amount</label>
              <div className="relative"><span className="absolute inset-y-0 left-0 pl-3 flex items-center text-neutral-400 dark:text-neutral-500 text-sm font-medium">{'\u20B1'}</span><input type="number" value={principalLoan} onChange={(e) => setPrincipalLoan(e.target.value)} className="form-input w-full pl-7" placeholder="0.00" min="0" max={((approveModal.item.fair_market_value || approveModal.item.appraised_value) * (loanSettings.ltv_ratio || 0.70)) || undefined} step="0.01" /></div>
              <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-1.5 flex items-center gap-1"><span className="material-symbols-rounded text-sm">info</span>Cannot exceed {((loanSettings.ltv_ratio || 0.70) * 100).toFixed(0)}% LTV — max: {formatCurrency((approveModal.item.fair_market_value || approveModal.item.appraised_value) * (loanSettings.ltv_ratio || 0.70))}.</p>
              {Number(principalLoan) > Number(approveModal.item.fair_market_value || approveModal.item.appraised_value) * (loanSettings.ltv_ratio || 0.70) && (<p className="text-xs text-red-500 mt-1 flex items-center gap-1"><span className="material-symbols-rounded text-sm">warning</span>Amount exceeds the LTV-based maximum loanable amount.</p>)}
            </div>
            <div><label className="form-label">Offered Amount (optional)</label><div className="relative"><span className="absolute inset-y-0 left-0 pl-3 flex items-center text-neutral-400 dark:text-neutral-500 text-sm font-medium">{'\u20B1'}</span><input type="number" value={offeredAmount} onChange={(e) => setOfferedAmount(e.target.value)} className="form-input w-full pl-7" placeholder="0.00" min="0" step="0.01" /></div></div>
            <div><label className="form-label">Storage Location (optional)</label><input type="text" value={storageLocation} onChange={(e) => setStorageLocation(e.target.value)} className="form-input w-full" placeholder="e.g. Vault A - Shelf 3 - Bin 12" /></div>
            <div className="flex items-center justify-end gap-3 pt-2">
              <button type="button" onClick={() => setApproveModal({ open: false, item: null })} className="btn-outline">Cancel</button>
              <button type="button" onClick={handleApprove} disabled={!principalLoan || Number(principalLoan) <= 0 || Number(principalLoan) > Number(approveModal.item.fair_market_value || approveModal.item.appraised_value) * (loanSettings.ltv_ratio || 0.70) || modalLoading} className="btn-primary disabled:opacity-40 disabled:cursor-not-allowed">
      ...
      <Modal open={rejectModal.open} onClose={() => setRejectModal({ open: false, item: null })} title="Reject Appraisal" size="sm">
        {rejectModal.item && (
          <div className="space-y-5">
            ...
            <div><label className="form-label">Reason (optional)</label><textarea value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} className="form-input w-full min-h-[100px] resize-y" placeholder="Provide a reason..." rows={4} /></div>
            <div className="flex items-center justify-end gap-3 pt-2">
              <button type="button" onClick={() => setRejectModal({ open: false, item: null })} className="btn-outline">Cancel</button>
              <button type="button" onClick={handleReject} disabled={modalLoading} className="px-4 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-semibold flex items-center gap-2 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
      ...
      {/* Decline Modal */}
      <Modal open={declineModal.open} onClose={() => setDeclineModal({ open: false, item: null })} title="Customer Declined" size="sm">
        {declineModal.item && (
          <div className="space-y-5">
            <div className="p-3 bg-neutral-50 dark:bg-neutral-700/30 rounded-lg space-y-1">
              <p className="text-sm font-semibold text-neutral-800 dark:text-white">{declineModal.item.general_desc}</p>
              <p className="text-xs text-neutral-500 dark:text-neutral-400">Appraised Value: <span className="font-bold text-primary">{formatCurrency(declineModal.item.appraised_value)}</span></p>
            </div>
            <div><label className="form-label">Reason (optional)</label><textarea value={declineReason} onChange={(e) => setDeclineReason(e.target.value)} className="form-input w-full min-h-[80px] resize-y" placeholder="Why did the customer decline?" rows={3} /></div>
            <div className="flex items-center justify-end gap-3 pt-2">
              <button type="button" onClick={() => setDeclineModal({ open: false, item: null })} className="btn-outline">Cancel</button>
              <button type="button" onClick={handleDecline} disabled={modalLoading} className="px-4 py-2 rounded-lg bg-neutral-600 hover:bg-neutral-700 text-white text-sm font-semibold flex items-center gap-2 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                {modalLoading ? (<><span className="material-symbols-rounded animate-spin text-lg">progress_activity</span>Processing...</>) : (<><span className="material-symbols-rounded text-lg">person_off</span>Mark as Declined</>)}
              </button>
            </div>
          </div>
        )}
      ...
      {/* Issue Ticket Modal */}
      {issueItem && <IssueTicketModal item={issueItem} onClose={() => setIssueItem(null)} onSuccess={handleIssueSuccess} />}
    </div>
  )
}
